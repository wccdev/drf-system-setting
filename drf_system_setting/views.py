...
class DictViewSet(
    ...
    @action(detail=False, url_path=r"(?P<parent_code>.+)/children")
    def children(self, request, parent_code, *args, **kwargs):
        queryset = self.get_queryset().filter(parent__code=parent_code)
